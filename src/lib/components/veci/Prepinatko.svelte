<script lang="ts">
	...
	import { nazevSHvezdou, type Prepinatkova } from '$lib/Vec';

	export let t: Translations;
	export let vec: Prepinatkova<D>;
	export let data: D;
</script>
		...
		<div class="btn-group" role="group">
			{#each vec.moznosti as moznost, i}
				<input
					...
					autocomplete="off"
				/>
				<label class="btn btn-sm btn-outline-secondary" for={vec.nazev(data) + Boolean(i)}
					>{t.get(moznost)}</label
				>
			{/each}
		</div>
