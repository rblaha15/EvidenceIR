...
export const pdfRKTL: GetPdfData<'RKTL'> = async ({ data, t, pump, lastYear, addDoc, lang, fetch }) => {
    ...
    const veci = checks.mapTo((rok, kontrola, i) => {
        if (!kontrola) return [];
        const k = valuesToRawData(defaultRKTL(rok, []), rawDataToValues(defaultRKTL(rok, []), kontrola)); // seřazení informací
    ...
    return {
        fileNameSuffix: `${data.IN.ir.cislo} ${pumpInfo.cislo} roky ${startYear}-${maxYear}`,
        ...start,
        ...metadata,
        ...veci,
    };
...
const pdfRKT: GetPdfData<'RKT'> = async ({ data, t, pump, lastYear, addDoc, lang, fetch }) => {
    ...
    const veci = checks.mapTo((rok, kontrola, i) => {
        if (!kontrola) return [];
        const k = valuesToRawData(defaultRKT(rok, []), rawDataToValues(defaultRKT(rok, []), kontrola)); // seřazení informací
